...
@section('content')
        ...
        <h1 class="h3 text-gray-800">Sifat Surat</h1>
            <div class="d-flex w-100">
                    <div class="action-buttons mt-4 mb-3">
                        <a href="{{ url('/createsifatsurat') }}" class="btn btn-md btn-success">Tambah</a>
                    </div>
            </div>
        </div>
        @include('tabel/tabelsifatsurat',$sifatsurat)
                
        @foreach ($sifatsurat as $data)
            <div class="modal fade" id="deleteModal{{$data->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    ...
                    <div class="modal-footer">
                        ...
                        <form action="{{ url('/deletesifatsurat/'.$data->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
